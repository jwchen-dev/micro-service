...
import jw.ms.server.handler.EventHandler;
...
public class Reactor {
    ...
    public Selector getReadDemultiplexer() {
        return readSelector;
    }
...
//                        new Thread((ReadEventHandler)handler).start();
                    ...
                    if (handle.isWritable()) {
                        Request request = (Request) handle.attachment();
                        Router.doProcess(request.getPath(), handle);
